...
use TreeData\objects\{Node, Root, Tree};
...
//        $this->ResetDrifter();
    ...
    public function TreeRecurse(PgSql $pg, $parent){
        ...
        foreach ($nodes as $nodeData) {
            ...
            if(count($data) > 0) {
                if (count($children) > 0)
                    $node = new Tree($currentCode, $data, $children);
                else
                    $node = new Node($currentCode, $data);
            }
            else
                $node = new Root($currentCode, $children);
